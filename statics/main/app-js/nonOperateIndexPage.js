...
// function chartajax() {
//     var t = $('#net_type').val();
//     $.getJSON("{% url 'nonoperate_chartajax' %}?type="+t, function(ret){
//         Highcharts.chart('dev-net', {
//         chart: {
//             type: 'column'
//         },
//         title: {
//             text: "{% trans '网络使用情况' %}("+ret.type+")"
//         },
//         /*subtitle: {
...
function user_chartajax() {
  // var t = $('#net_type').val();
  // "{% url 'nonoperate_user_chartajax' %}"
  $.getJSON(`${ url }/nonoperate/nonoperate_index/nonoperate_user_chartajax/`, {
    'type': ''
  }, function(ret) {
    // console.log(ret);
...
function ssid_select() {
  // "{% url 'nonoperate_ssid_select' %}"
  $.getJSON(`${ url }/nonoperate/nonoperate_index/nonoperate_ssid_select/`, {}, function(ret) {
    // console.log(ret);
    if (ret.length === 0) {
...
function user_counterajax() {
  ...
  // "{% url 'nonoperate_user_counterajax' %}"
  $.getJSON(`${ url }/nonoperate/nonoperate_index/nonoperate_user_counterajax/`, {
    'type': type
  }, function(ret) {
    // console.log(ret);
...
function sort_change() {
  ...
  // "{% url 'nonoperate_sort_type_change' %}?type=" + t
  $.getJSON(`${ url }/nonoperate/nonoperate_index/nonoperate_sort_type_change/?type=${ t }`, function(ret) {
    var h = '';
    var g = '';
    // console.log('123', ret);
    if (t === 1) {
...
function ap_warningajax() {
  ...
  // "{% url 'nonoperate_warningajax' %}?type=" + t
  $.getJSON(`${ url }/nonoperate/nonoperate_index/nonoperate_warningajax/?type=${ t }`, function(ret) {
    var h = '';
    // console.log(ret);
    if (ret.length !== 0) {
      for (var i = 0; i < ret.length; i++) {
