...
function al_access_table() {
  ...
  oTable = $('#datatable-default').DataTable({
    ...
    "ajax": `${ url }/nonoperate-list/nonoperate_already_access_ajax/`,
    "columns": columns,
    'createdRow': function(row, data, index) {
      ...
      if ($.trim(data.name) !== "") {
        // "<a href=\"#\" onclick=\"ajaxfunc('" + data.mac + "','" + data.state + "');wlan_modify(\'" + data.id + "\');\" class=\"table-a-color\">" + data.name + "</a>"
        $('td', row).eq(resultColumn.indexOf('name')).html(`<a href="#" onclick="ajaxfunc('${ data.mac }','${ data.state }');wlan_modify('${ data.id }');" class="table-a-color">${ data.name }</a>`);
      } else {
        // "<a href=\"#\" onclick=\"ajaxfunc('" + data.mac + "','" + data.state + "');wlan_modify(\'" + data.id + "\');\" class=\"table-a-color\">" + data.model + "_" + (data.mac).substring(6) + "</a>"
        $('td', row).eq(resultColumn.indexOf('name')).html(`<a href="#" onclick="ajaxfunc('${ data.mac }','${ data.state }');wlan_modify('${ data.id }');" class="table-a-color">${ data.model }_${ (data.mac).substring(6) }</a>`);
      ...
      if (data.state === pageWords.online) {
        if (data.admin_power_control !== "ban") {
          // a + "<a class=\"btn btn-default table-btn-btnpad\" href=\"#\" onclick=\"ap_reboot('" + data.mac + "');\">{% trans '重启' %}</a>"
          a = `${ a }<a class="btn btn-default table-btn-btnpad" href="#" onclick="ap_reboot('${ data.mac }');">${ pageWords.reboot }</a>`;
          if (data.upgrade_button === true) {
            // a + "<a class=\"btn btn-default table-btn-btnpad\" href=\"#\" title=\"" + data.upgrade_version + "\" onclick=\"ap_upgrade('" + data.mac + "');\">{% trans '升级' %}</a>"
            a = `${ a }<a class="btn btn-default table-btn-btnpad" href="#" title="${ data.upgrade_version }" onclick="ap_upgrade('${ data.mac }');">${ pageWords.update }</a>`;
          }
        }
      }
      // "{% trans '重启' %}" "{% trans '升级' %}"
      if (data.state === pageWords.reboot || data.state === pageWords.update) {
        ...
        // a + "<a class=\"btn btn-default table-btn-btnpad\" href=\"#\" onclick=\"ap_del('" + data.mac + "');\">{% trans '移除' %}</a>"
        a = `${ a }<a class="btn btn-default table-btn-btnpad" href="#" onclick="ap_del('${ data.mac }');">${ pageWords.remove }</a>`;
      }
...
//             // }
//         },
//         "ajax": "{% url 'nonoperate_wait_access_ajax' %}",
//         "columns": [
//             { "data": "model" },
//             { "data": "mac" },
//             { "data": "lastip" },
...
//             $('td',row).eq(6).html("<a class=\"btn btn-default table-btn-btnpad\" href=\"{% url 'nonoperate_access' %}?mac="+data.mac+"\">{% trans '准入' %}</a>");
//         }
//      });
...
function eventajax() {
  ...
  if (type == "admin") {
    // "{% url 'nonoperate_eventajax' %}?type=" + type + "&time=" + time
    table.ajax.url(`${ url }/nonoperate-list/nonoperate_eventajax/?type=${ type }&time=${ time }`).load();
  };
  if (type == "dev") {
    // "{% url 'nonoperate_eventajax' %}?type=" + type + "&time=" + time
    table.ajax.url(`${ url }/nonoperate-list/nonoperate_eventajax/?type=${ type }&time=${ time }`).load();
...
function ap_upgrade(mac) {
  // body...
  // "{% url 'nonoperate_update' %}"
  $.post(`${ url }/nonoperate-list/nonoperate_update/`, {
    'mac': mac
  }, function(ret) {
