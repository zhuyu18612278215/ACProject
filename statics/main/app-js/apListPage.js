...
function al_access_table() {
  ...
  oTable = $('#datatable-default').DataTable({
    ...
    "serverSide": true,
    // "{% url 'ap_already_access_ajax' %}"
    "ajax": `${ url }/ap-list/ap_already_access_ajax/`,
    "columns": columns,
    'createdRow': function(row, data, index) {
      ...
      if (data.state === pageWords.online) {
        if (data.admin_power_control !== "ban") {
          // a = a + "<a class=\"btn btn-default table-btn-btnpad\" href=\"#\" onclick=\"ap_reboot('" + data.mac + "');\">{% trans '重启' %}</a>";
          ...
          if (data.upgrade_button === true) {
            // a = a + "<a class=\"btn btn-default table-btn-btnpad\" href=\"#\" title=\"" + data.upgrade_version + "\" onclick=\"ap_upgrade('" + data.mac + "');\">{% trans '升级' %}</a>";
      ...
      if (data.state === pageWords.reboot || data.state === pageWords.update) {
        ...
        // a = a + "<a class=\"btn btn-default table-btn-btnpad\" href=\"#\" onclick=\"ap_del('" + data.mac + "');\">{% trans '移除' %}</a>";
...
//             $('td',row).eq(6).html("<a class=\"btn btn-default table-btn-btnpad\" href=\"{% url 'ap_access' %}?mac="+data.mac+"\">{% trans '准入' %}</a>");
//         }
//      });
...
function eventajax() {
  ...
  if (type === "admin") {
    // "{% url 'ap_eventajax' %}?type=" + type + "&time=" + time
    table.ajax.url(`${ url }/ap-list/ap_eventajax/?type=${ type }&time=${ time }`).load();
  };
  if (type === "dev") {
    // "{% url 'ap_eventajax' %}?type=" + type + "&time=" + time
    table.ajax.url(`${ url }/ap-list/ap_eventajax/?type=${ type }&time=${ time }`).load();
