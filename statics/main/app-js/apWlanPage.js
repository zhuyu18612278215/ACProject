...
function wlan_ajax() {
  ...
  if (document.getElementById('safeType').value === 'ccmp' || document.getElementById('safeType').value === 'tkip' || document.getElementById('safeType').value === 'eap') {
  ...
  } else if (document.getElementById('safeType').value === "open") {
    $('#passphrase').val("");
  }
  if (document.getElementById('safeType').value === 'eap') {
    if ($.trim($('#eapIP').val()) === '' || $.trim($('#eapPort').val()) === '') {
      alert(pageWords.eapIPPortEmptyError);
      sign = 0;
    }
  }
...
$("#safeType").on("change", function() {
  let value = document.getElementById('safeType').value;
  if (value === 'open') {
    $('#passphrase_hide').hide();
    $('#eapIP_hide').hide();
  } else if (value === 'tkip' || value === 'ccmp') {
...
$("#safeType_m").on(" change", function() {
  let value = document.getElementById('safeType_m').value;
  if (value === 'open') {
...
function get_auto_accept() {
  // body...
  // '{% url "ap_auto_accept" %}'
  $.post(`${ url }/ap/ap_wlan/auto-accept/`, {
    'accept': 'get'
  }, function(ret) {
...
function modify_wlan_ajax(pk) {
  ...
  if (document.getElementById('safeType_m').value === 'ccmp' || document.getElementById('safeType_m').value === 'tkip' || document.getElementById('safeType_m').value === 'eap') {
  ...
  } else if (document.getElementById('safeType_m').value === "open") {
    $('#passphrase_m').val("");
  }
  if (document.getElementById('safeType_m').value === 'eap') {
    if ($.trim($('#eapIP_m').val()) === '' || $.trim($('#eapPort_m').val()) === '') {
      alert(pageWords.eapIPPortEmptyError);
      sign = 0;
    }
  }
  ...
  if (sign != 0) {
    $.ajax({
      type: "POST",
      dataType: "html",
      // "{% url 'modify_wlan_ajax' %}?pk=" + pk
      url: `${ url }/ap/ap_wlan/modify_wlan_ajax/?pk=${ pk }`,
...
function black_list_add_table() {
  ...
  // "{% url 'black_list_add_ajax' %}"
  table.ajax.url(`${ url }/ap/ap_wlan/black_list_add_ajax/`).load();
}
...
$('#blacklist_add').click(function() {
  /* Act on the event */
  // '{% url "blacklist_add" %}'
  $.post(`${ url }/ap/ap_wlan/blacklist_add/`, {
...
function black_list_table() {
  ...
  // "{% url 'black_list_table' %}"
  table.ajax.url(`${ url }/ap/ap_wlan/black_list_table/`).load();
}

function blacklist_remove(mac) {
  // body...
  // '{% url "blacklist_remove" %}'
  $.post(`${ url }/ap/ap_wlan/blacklist_remove/`, {
    'mac': mac
  }, function(ret) {
    /*optional stuff to do after success */
    alert(ret.error);
...
function open_form1(wl_id) {
  ...
  // "{% url 'ap_wlan_info_ajax' %}?id=" + wl_id
  $.getJSON(`${ url }/ap/ap_wlan/ap_wlan_info_ajax/?id=${ wl_id }`, function(ret) {
    // console.log(ret);
    var x = '';
    ...
    if ($.trim(ret['wl'].encry_type) === '') {
      document.getElementById('safeType_m').value = 'open';
      $('#passphrase_hide_m').hide();
      $('#passphrase_m').val("");
      $('#eapIP_hide_m').hide();
      $('#eapIP_m').val('');
      ...
      document.getElementById('safeType_m').value = 'tkip';
      $('#passphrase_hide_m').show();
      $('#passphrase_m').val(ret['wl'].passphrase);
      $('#eapIP_hide_m').hide();
      $('#eapIP_m').val('');
      $('#eapPort_m').val('1812');
    } else if (ret['wl'].encry_type === 'ccmp' && ret['wl'].sec_type !== 'eap') {
      document.getElementById('safeType_m').value = 'ccmp';
      $('#passphrase_hide_m').show();
      $('#passphrase_m').val(ret['wl'].passphrase);
      $('#eapIP_hide_m').hide();
      $('#eapIP_m').val('');
      ...
      document.getElementById('safeType_m').value = 'eap';
      $('#passphrase_hide_m').show();
      $('#passphrase_m').val(ret['wl'].passphrase);
      $('#eapIP_hide_m').show();
      $('#eapIP_m').val(ret['wl'].eapIP);
      $('#eapPort_m').val(ret['wl'].eapPort);
    };
...
function choose_ap_group_ajax() {
  // body...
  // "{% url 'choose_ap_group_ajax' %}"
  $.getJSON(`${ url }/ap/ap_wlan/choose_ap_group_ajax/`, function(ret) {
    // console.log(ret);
    // console.log(ret);
